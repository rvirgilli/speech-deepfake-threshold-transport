...
Order-statistic calibration is established: Neyman--Pearson methods control type-I error through class-0 order statistics \cite{tong18} and negative-only conformal calibration provides marginal guarantees \cite{vovk}. In audio, Zhao et al.\ calibrate a bona-fide-only $\alpha$-quantile \cite{zhao26ca}, \cite{eerhides26} audits threshold transfer and unlabeled corrections, and \cite{kwok25} documents bona-fide-driven cross-testing errors. Relative to that audit (one detector, two target corpora), we add a 108-cell fixed-FPR map with real transmissions, a finite-$N$ target-bona-fide comparison, and a 264-pair disjoint replication. Auditing both error rates at a preset threshold is established in spoofing evaluation \cite{asvspoof5}, and \cite{rubio26} holds a cost-optimal threshold fixed across the same 21LA codecs but reports relative cost only. Pham et al.\ \cite{pham26} study dataset-specific versus inference thresholds and bona-fide resource effects. Khan et al.'s TRACE \cite{trace26} contrasts transferred and target-selected thresholds for partial deepfakes; we measure both error rates at a fixed bona-fide-error target and evaluate finite-sample target-bona-fide calibration. Demographic threshold calibration \cite{fursule26} likewise separates class-conditional error changes from unchanged discrimination. ReplayDF \cite{muller25replay} reports channel-induced spoof-side degradation with stable bona-fide acceptance; we measure it at a fixed bona-fide error target. Sch\"afer and Steinebach \cite{schaefer26} evaluate five detectors on social-media audio at thresholds set in advance and report large degradation relative to EER-based thresholds; we study that setting at a fixed bona-fide error target and separate which of the two error rates moves.
...
\section{Conclusion}
\begin{sloppypar}
Channel-matched training reduced checkpoint-median drift in the tested recipes: SSL-AASIST run medians separated completely; AASIST decreased descriptively at one seed per arm. The registered AASIST material-reduction rule (Table~\ref{tab:fnr} release) requires arm~2's selected-checkpoint $K$ range entirely below arm~1's and a median decrease exceeding the wider within-arm range width. AASIST fails: its median decrease is 8, below the wider range width of 9. The controls have unequal starting costs: maximum excess FNR fell for SSL-AASIST but rose for AASIST. Training qualifies blanket recalibration advice; costly errors may persist, and costly transfer still warrants target-channel recalibration. Under exchangeability without ties, a target-channel bona-fide quantile provides marginal expected control; dispersion depends on the sampling unit, and it cannot repair class overlap.
\end{sloppypar}
...
competing interests.
The authors used a large language model (Anthropic Claude) to help write and
edit the text, the LaTeX layout and the supporting code, reviewed all assisted
content, and take full responsibility.

\section{Compliance with Ethical Standards}
This study used only previously collected, publicly available recordings from
the ASVspoof, In-the-Wild and BRSpeech-DF corpora, under their licences and
