...
\begin{abstract}
A speech-deepfake detector's threshold, set to 5\% false-positive rate (FPR, bona fide rejected) on one channel, often loses that operating point on another. Across 108 detector, source and target combinations built from the real transmissions of ASVspoof~2021~LA and four corpora, 77 realize an FPR more than $2\times$ off target (36 above, 41 below). The conservative misses are the quiet ones: 41 of the 72 cells inside a $\pm5$-point FPR tolerance sit below half the target, so the tolerance reads clean while spoofs pass. In one AASIST case, a threshold calibrated on PSTN and deployed on the same recordings over G.722 gives a mean \textbf{0.42\% FPR} while missing a mean \textbf{70\% of spoofs, against 0.66\% at the deployment-calibrated threshold}. The failure persists on ASVspoof~5 with no shared recordings or speakers: 187 of 264 pairs miss by more than $2\times$, and on the 66 usable SSL-AASIST pairs the transported threshold misses a median 41.5 points more spoofs. Recalibrating on 500 target bona-fide recordings with an order statistic restores mean FPR to 4.91--5.06\% across twelve detector--corpus cells at a mean FNR cost of at most 0.5 points in usable cells, but the guarantee is marginal, not per deployment: speaker clustering widens the realized-FPR spread in all six tested cells. Label-free alternatives, an importance-weighted quantile and two drift monitors, neither restore nor reliably detect the loss of control. Threshold transport can therefore fail quietly, and fixed-threshold deployments should report both error rates, the calibration sampling unit and its dispersion.
\end{abstract}
...
C5 neighbour norm & 1.4 & 6.4 & 0.0 & 0.2 \\
Cohort z-norm & 7.4 & 20.3 & 11.9 & 23.2 \\
Gaussian q. & 5.6 & 11.9 & 5.7 & 0.0 \\
\multicolumn{5}{l}{\emph{Mean FNR (\%) of the two labeled competitors, same runs}} \\
Cohort z-norm & 0.2 & 0.9 & 10.7 & 73.1 \\
Gaussian q. & 0.2 & 2.8 & 17.2 & 100.0 \\
