...
\section{Conclusion}
A below-target FPR can hide missed spoofs: AASIST on PSTN$\to$G.722 misses 70\% of spoofs at 0.42\% FPR; a target-bona-fide oracle misses 0.66\%. On disjoint ASVspoof~5 data, excess FNR concentrates in transfers from overlap-dominated sources, yet 11 of 30 usable-source transfers exceed $+10$. Channel-matched training reduced checkpoint-median drift, but AASIST fails the pre-specified rule requiring arm~2's five-checkpoint $K$ range entirely below arm~1's and a median decrease exceeding the wider range width: 8 falls short of 9. The controls have unequal starting costs: maximum excess FNR fell for SSL-AASIST but rose for AASIST, so costly transfer still warrants recalibration. Under exchangeability without ties, a target-channel bona-fide quantile provides marginal expected control; dispersion depends on the sampling unit, and it cannot repair class overlap.
