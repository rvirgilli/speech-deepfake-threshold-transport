...
\caption{Recoverable spoof loss by source usability. Each point is a deployment cell: $x=\log_2(\max(\mathrm{FPR},3/n)/5\%)$ (\S4; dotted $2\times$ bars); $y$ is mean transported FNR minus all-target-bona-fide oracle FNR (points). Blue circles/orange squares: 108 SSL-AASIST/AASIST 21LA and cross-corpus cells. Triangles: 132 ASVspoof~5 SSL-AASIST pairs; hollow, overlap-dominated destinations; filled, usable destinations (oracle FNR $\le$50\% for every source). Filled green upward/gray downward triangles distinguish 30 usable-source/36 weak-source transfers (same criterion); black outlines identify the 11 usable-source transfers exceeding $+10$.}
\label{fig:drift}
\end{figure}
\label{sec:exp}
