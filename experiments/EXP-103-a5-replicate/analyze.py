...
Two arms are reported separately and never pooled:
  ...
  twin-free   every unprocessed bona-fide recording plus the processed versions
              of the 80.5% of sources that appear under exactly one codec
              -> the replicate proper
  crossed     the processed versions of the 19.5% of sources that appear under
              all 11 codecs -> 21LA's twin structure on the same corpus, which
              measures what the twinning is worth
...
A5 = Path(os.environ.get("A2_DATA", Path.home() / "data/corpora/anti-spoofing")) / "asvspoof5"
PROTO = Path(os.environ.get("A2_A5_PROTO", A5 / "ASVspoof5.eval.track_1.tsv"))
...
def load_scores(model):
    """Scores from the raw scoring-run arrays when present, otherwise from the
    released score table artifacts/scores_<model>_asv5_eval.csv.gz."""
    out = {}
    files = sorted(glob.glob(str(RAW / model / "*.npz")))
    if not files:
        with gzip.open(OUT / f"scores_{model}_asv5_eval.csv.gz", "rt") as f:
            return {r["utt_id"]: float(r["score"]) for r in csv.DictReader(f)}
    for f in files:
...
def build():
    """Rows of the evaluation protocol and the set of bona-fide sources that
    appear under exactly one processed codec. The twin-free roster keeps every
    unprocessed bona-fide recording (protocol source field '-') plus the
    processed versions of those single-codec sources; sources under several
    codecs contribute only their unprocessed row."""
    rows = [l.split() for l in open(PROTO)]
    processed = defaultdict(set)
    for r in rows:
        if r[8] == "bonafide" and r[5] != "-":
            processed[r[5]].add(r[3])
    one_codec = {s for s, c in processed.items() if len(c) == 1}
    return rows, one_codec
...
def cells(rows, scores, twin_free, arm):
    ...
    for r in rows:
        spk, utt, cond, lab = r[0], r[1], r[3], r[8]
        in_twin_free = r[5] == "-" or r[5] in twin_free
        if lab == "bonafide" and in_twin_free != (arm == "twin_free"):
            continue
        s = scores.get(utt)
        if s is None:
            continue
...
def main():
    ...
    print(f"single-codec bona-fide sources: {len(twin_free):,}")
    allres = {}
